...
class CycleGanDataset(keras.utils.Sequence):
    def __init__(self, annotation_lines_A, annotation_lines_B, input_shape, batch_size):
    ...
    def __len__(self):
        return math.ceil(max(self.length_A, self.length_B) / float(self.batch_size))
    
    def on_epoch_end(self):
        shuffle(self.annotation_lines_A)
        shuffle(self.annotation_lines_B)
    ...
    def __getitem__(self, index):
        ...
        for i in range(index * self.batch_size, (index + 1) * self.batch_size):  
            ...
            image_A = preprocess_input(np.array(image_A, dtype=np.float32))
            ...
            image_B = preprocess_input(np.array(image_B, dtype=np.float32))
            images_B.append(image_B)
        return np.array(images_A, dtype=np.float32), np.array(images_B, dtype=np.float32)
